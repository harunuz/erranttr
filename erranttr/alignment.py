...
import numpy as np
from zemberek.core.turkish.turkish_alphabet import TurkishAlphabet as TR_ALPH
...
class Alignment:
    ...
    _open_pos = {PPOS.Adjective, PPOS.Adverb, PPOS.Noun, PPOS.Verb}
    ...
    def __init__(
            self,
            orig: Union['SentenceAnalysis', Sequence['SentenceWordAnalysis']],
            cor: Union['SentenceAnalysis', Sequence['SentenceWordAnalysis']],
            lev=False
    ):
        # Set orig and cor
        self.orig = orig
        ...
        self.o_low = [tu.lower(o.word_analysis.inp) for o in self.orig]
        self.c_low = [tu.lower(c.word_analysis.inp) for c in self.cor]
    ...
    def align(self, lev):
        ...
        for i in range(o_len):
            for j in range(c_len):
                # Matches
                if self.orig[i].word_analysis.inp == self.cor[j].word_analysis.inp:
                ...
                # Non-matches
                    ...
                    # Linguistic Damerau-Levenshtein
                        ...
                        while i - k >= 0 and j - k >= 0 and \
                            ...
                            if sorted(self.o_low[i - k:i + 1]) == sorted(self.c_low[j - k:j + 1]):
    ...
    def get_sub_cost(self, o: 'SentenceWordAnalysis', c: 'SentenceWordAnalysis'):
        # Short circuit if the only difference is case
        if tu.lower(o.word_analysis.inp) == tu.lower(c.word_analysis.inp):
            return 0.
        # Lemma cost
        ...
        if o.best_analysis.item.primary_pos == c.best_analysis.item.primary_pos:
            if o.best_analysis.item.secondary_pos == c.best_analysis.item.secondary_pos:
                ...
                pos_cost = 0.15
        elif o.best_analysis.item.primary_pos in self._open_pos and c.best_analysis.item.primary_pos in self._open_pos:
            pos_cost = 0.25
        else:
            pos_cost = 0.5
        # Char cost
        char_cost = Indel.normalized_distance(o.word_analysis.inp, c.word_analysis.inp)
        # Combine the costs
